...
pub async fn set_difficulty_thresholds(
        ...
        || body.score_moderate_max <= 0
    {
        ...
        return Err(UsecaseError::Validation(
            "All threshold values must be positive".to_string(),
        ));
    }
    ...
    if body.distance_easy_max_km >= body.distance_moderate_max_km {
        ...
        return Err(UsecaseError::Validation(
            "Easy distance threshold must be less than moderate".to_string(),
        ));
    }
    ...
    if body.elevation_easy_max_m >= body.elevation_moderate_max_m {
        ...
        return Err(UsecaseError::Validation(
            "Easy elevation threshold must be less than moderate".to_string(),
        ));
    }
    ...
    if body.score_easy_max >= body.score_moderate_max {
        ...
        return Err(UsecaseError::Validation(
            "Easy score threshold must be less than moderate".to_string(),
        ));
    }

    state
        .settings_usecase
        .set_difficulty_thresholds(&body)
        .await
        .map_err(|e| UsecaseError::Internal(e.to_string()))?;
