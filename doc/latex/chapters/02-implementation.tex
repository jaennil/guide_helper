...
\begin{itemize}
  \item \textbf{geocode}~--- поиск географических координат по текстовому запросу через Nominatim API;
  \item \textbf{search\_routes}~--- поиск маршрутов в каталоге по тексту, категории и местоположению;
  \item \textbf{get\_route\_details}~--- получение подробной информации о конкретном маршруте по UUID;
  \item \textbf{get\_weather}~--- запрос прогноза погоды для заданных координат через Open-Meteo.
\end{itemize}
...
\begin{itemize}
  \item \textbf{MapPage}~--- главная страница с интерактивной картой и редактором маршрутов;
  \item \textbf{ExplorePage}~--- каталог публичных маршрутов с поиском и фильтрацией;
  \item \textbf{ProfilePage}~--- профиль пользователя со списком созданных маршрутов;
  \item \textbf{BookmarksPage}~--- список маршрутов, добавленных в избранное;
  \item \textbf{ChatPage}~--- интерфейс ИИ-ассистента;
  \item \textbf{AdminPage}~--- административная панель управления;
  \item \textbf{SharedRoutePage}~--- публичный просмотр маршрута по уникальной ссылке.
\end{itemize}
