...
slides = {
    2: (
        ...
        [
            ("Проблема", [
                "маршрут, фотографии и описание обычно хранятся в разных местах",
                "автору сложно быстро собрать цельный материал для клиента",
                "турист получает не живой маршрут, а набор разрозненных ссылок",
            ]),
        ],
    ),
    3: (
    ...
    4: (
        ...
        [
            ("Вывод анализа", [
                "существующие сервисы закрывают только часть сценария",
                "нет единого контура: карта, фото, описание, публикация и реакции",
                "Guide Helper ориентирован на полный цикл подготовки фотомаршрута",
            ]),
        ],
    ),
    5: (
        ...
        [
            ("Формализация", [
                "37 функциональных требований реализованы и проверены",
                "выделены роли: автор маршрута, турист, администратор",
                "для защиты подготовлена трассировка «требование → реализация → проверка»",
            ]),
        ],
    ),
    6: (
    ...
    10: (
        ...
        [
            ("Расширение сценария", [
                "AI-ассистент помогает искать места и строить маршруты",
                "исторический таймлайн показывает контекст карты во времени",
                "погода, высоты и сложность помогают оценить прохождение",
            ]),
        ],
    ),
    11: (
        ...
        [
            ("Проверка качества", [
                ...
                "CI/CD собирает и доставляет сервисы в Kubernetes",
            ]),
        ],
    ),
    12: (
        ...
        [
            ("Полученный результат", [
                ...
                "подготовлены демонстрационный стенд, документация и материалы защиты",
            ]),
        ],
    ),
}
...
def update_title_slide(root: ET.Element) -> None:
    for sp in text_shapes(root):
        ...
        if "09.03.01" in text:
            ...
            set_shape_text(sp, ["Москва, 18 мая 2026"], size=850)
    sp_tree = root.find(".//p:spTree", NS)
